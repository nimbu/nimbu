...
class Nimbu::Client
  ...
  # Run a rake command on the Nimbu app and return all output as
  # a string.
...
Check the output of "nimbu ps" and "nimbu logs" for more information.
    ERROR
  end
  ...
  def run_console_command(url, command, prefix=nil)
    ...
    if e.http_code == 422
      Nimbu::Command.extract_error(e.http_body, :raw => true)
    else
      raise e
    end
  end
  ...
  def default_resource_options_for_uri(uri)
    if ENV["HEROKU_SSL_VERIFY"] == "disable"
      {}
    elsif realize_full_uri(uri) =~ %r|^https://api.nimbu.com|
